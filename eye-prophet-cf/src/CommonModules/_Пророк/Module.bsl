...
Функция ПолучитьПрогноз(ТаблицаИстории, ГоризонтПланирования,
	...
	имяСервера = ИмяСервера(СтруктураПараметров);
	структURI = _ОбщегоНазначения.СтруктураURI(имяСервера);

	Если НРег(структURI.Схема) = "https" Тогда
	...
	Соединение = Новый HTTPСоединение(структURI.Хост, структURI.Порт, структURI.Логин, структURI.Пароль, , , защищенноеСоединение);
...
Функция ИмяСервера(СтруктураПараметров)
	
	Если СтруктураПараметров.Свойство("ИмяСервера")
			И ЗначениеЗаполнено(СтруктураПараметров.ИмяСервера) Тогда

		Возврат СтруктураПараметров.ИмяСервера;
